...
/// <summary>
/// Selects the sum of user_score as sum_score from schema_with_foreign_keys.users.
/// </summary>
public sealed record WholeSetSumOfNullableScoreQuery
{
    ...
    public IStoredTableDataSet Result =>
        new StoredTableDataSet(
            new Table(
                new EmptyString(),
                [new Column(new String("sum_score"), new DoubleColumnType())],
                []
            ),
            [
                new Row(
                    new Dictionary<KeyValuePair<IColumn, ICell>, IColumn, ICell>(
                        [
                            new KeyValuePair<IColumn, ICell>(
                                new Column(
                                    new String("sum_score"),
                                    new DoubleColumnType()
                                ),
                                new InvariantCell(new Double(98))
                            ),
                        ],
                        pair => pair.Key,
                        pair => pair.Value,
